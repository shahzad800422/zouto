...
<h3>Parcel Products</h3>
<button type="button" class="btn btn-pdf" name="button">Generate PDF</button>
<table id="example14" class="display" style="width:100%">
    ...
    <tbody>
        ...
        $sql = 'SELECT * FROM customer_product_wishlist WHERE parcel_number IS NOT NULL AND invoiced=1 GROUP BY parcel_number';
        $res = $con->query($sql);
        if ($res->num_rows > 0) {
            $counter = 1;
            foreach ($res as $result) {
                $pn = $result['parcel_number'];
                $sql2 = "SELECT id, title, id_customer, price, net_price, qty, hs_code, supplier_track_number FROM customer_product_wishlist WHERE parcel_number='$pn'";
                $res2 = $con->query($sql2);
                $tit = [];
                $ids = [];
                if ($res2->num_rows > 0) {
                    ...
                    foreach ($res2 as $result2) {
                        ...
                        $sql3 = "SELECT * FROM shopify_customers WHERE id_customer='$cstt'";
                        $result3 = $con->query($sql3);
                        $get_data = mysqli_fetch_assoc($result3);
                        $pri += $result2['price'];
                        ...
                        $test .= "<span data-name='$nam' data-price='$prr' data-hs='$hs' data-qty='$qty' data-supp_tracking='$supplier_track_number'>$nam<b>&nbsp;($fname $lname - $idcs)</b></span><br>";
                    }
...
<div class="pdf_container" style="display:none">
    <div class="row pdf">
        ...
        <!--        <h2 class="fw-b">Electro Trade</h2>-->
        <!--    </div>-->
        <!--    <div class="col-md-6 text-right" style="width: 50%">-->
        <!--        <h2 class="mb-0 fw-b">Facture</h2>-->
        <!--        <h3 class="mt-0 fw-b">Réf. : FA2112-0005</h3>-->
        <!--    </div>-->
        <!--    <div class="col-md-5" style="width: 41.66666667%;">-->
        <!--        <h7>Émetteur</h7>-->
        <!--        <div class="col-md-12 back-col" style="width: 100%">-->
        <!--            <h3 class="fw-b mt-0 mb-0">Electro Trade</h3>-->
        <!--            <h5 class="mt-0 mb-0">6 rue d'Armaillé</h5>-->
        <!--            <h5 class="mt-0 mb-0">75017 Paris</h5>-->
        <!--            <h5 class="mt-0 mb-0">France</h5>-->
        <!--            <h5>Tél.: 0755549247</h5>-->
        <!--        </div>-->
        <!--    </div>-->
        <!--    <div class="col-md-7" style="width: 58.33333333%;">-->
        <!--        <h7>Adressé à</h7>-->
        <!--        <div class="col-md-12 bord-col" style="width: 100%">-->
        <!--            <h3 class="fw-b mt-0 mb-0">Colisrael</h3>-->
        <!--            <h5 class="mt-0 mb-0">Yeshuat David 12 Modiin Ilit</h5>-->
        <!--            <h5 class="mt-0 mb-0">7180602</h5>-->
        <!--            <h5 class="mt-0 mb-0">Israël</h5>-->
        <!--            <h5 class="mt-0 mb-0">Numéro TVA: 328871694</h5>-->
        <!--        </div>-->
        <!--    </div>-->
        <!--</div>-->
        <table style="margin-bottom: 20px;">
            <thead>
                <tr>
                    ...
                    <th style="font-size: 30px;border:unset;text-align: right;">Facture<br><span style="font-weight: normal !important;font-size: 15px !important">INV<?php echo date("ymd"); ?> - <span class="pdf_count"></span></span></th>
                </tr>
            </thead>
            <tbody>
                <tr>
...
<script type="text/javascript">
    ...
    $(document).on("click", "#supplier_tracking_number", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ env('APP_URL') }}/update_supplier_track_number.php",
            data: {
                id: id,
                sup_num: sup_num
            },
    ...
    $('.shipped_status').on('change', function() {
        ...
        $.ajax({
            type: "POST",
            url: '{{ env('
            APP_URL ') }}/update_shipped_status.php',
            data: {
                id,
                status
            },
    ...
    function update_status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: '{{ env('
            APP_URL ') }}/update_parcel_status_backend.php',
            data: {
                id: id,
                status: status
            },
    ...
    function deleteData(id) {
        $.ajax({
            type: "POST",
            url: '{{ env('
            APP_URL ') }}/delete_parcel_backend.php',
            data: {
                id: id
            },
            success: function(data) {
        ...
        $('.checks').each(function() {
            if ($(this).is(':checked')) {
                ...
                $(this).closest('tr').next().find('td span').each(function() {
                    ...
                    $('.pdf_body').append('<tr><td>' + $(this).data('supp_tracking') + '</td><td>' + $(this).data('hs') + '</td><td>' + $(this).data('name') + '</td><td class="fl-right">' + $(this).data('qty') + '</td><td class="fl-right">' + $(this).data('price') + '</td><td>' + ($(this).data('qty') * $(this).data('price')).toFixed(2) + '</td></tr>')
                })

            }
        })
        $('.pdf_body').append('<tr><th>Total Price</th><th></th><th></th><th></th><th></th><th>' + tot_pri.toFixed(2) + '</th></tr>')
        $('ul.nav.nav-tabs, h3, .btn-pdf, #example14').hide()
        $('.pdf_container').show()
        window.print();
        $('ul.nav.nav-tabs, h3, .btn-pdf, #example14').show()
        $('.pdf_container').hide();
        // location.reload()
        window.location.href = window.location.origin + window.location.pathname + '?tab=' + $('ul.nav.nav-tabs li.active').data('tab')
    })
