...
<table id="example14" class="display" style="width:100%">
    ...
    <tbody>
        ...
        if (count($colisrealParcel['products']) > 0) {
            $counter = 1;
            foreach ($colisrealParcel['products'] as $result) {
                ...
                $res2 = Helper::dbQuery("SELECT id, title, id_customer, price, parcel_l, parcel_b, parcel_h, weight, net_price, tracked_number, parcel_number, qty, hs_code, supplier_track_number FROM customer_product_wishlist WHERE parcel_number='$pn' AND is_archived IS NULL");
                $tit = [];
                $ids = [];
                if (count($res2) > 0) {
