...
<table id="example14" class="display" style="width:100%">
    ...
    <tbody>
        ...
        $sql = 'SELECT * FROM customer_product_wishlist WHERE parcel_number IS NOT NULL AND is_archived IS NULL GROUP BY parcel_number';
        $res = $con->query($sql);
        if ($res->num_rows > 0) {
            $counter = 1;
            foreach ($res as $result) {
                $pn = $result['parcel_number'];
                $sql2 = "SELECT id, title, id_customer, price, parcel_l, parcel_b, parcel_h, weight, net_price, qty, hs_code, supplier_track_number FROM customer_product_wishlist WHERE parcel_number='$pn' AND is_archived IS NULL";
                $res2 = $con->query($sql2);
                $tit = [];
                $ids = [];
                if ($res2->num_rows > 0) {
                    ...
                    foreach ($res2 as $result2) {
                        ...
                        $test .= "<span data-name='$nam' data-price='$prr' data-hs='$hs' data-qty='$qty' data-supp_tracking='$supplier_track_number' data-l='$parcel_l' data-b='$parcel_b' data-h='$parcel_h' data-weight='$weight'>$nam<b>&nbsp;($fname $lname - $idcs)</b></span><br>";
                    }
