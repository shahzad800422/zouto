...
<section class="qwqw">
    ...
    <div class="row">
        <div class="" style="padding-top: 20px;text-align: left;padding-left: 20px">
            <a href="{{ env('APP_URL') }}/index2_customer?id=<?php echo $_GET['id_customer']; ?>&cart=<?php echo $_GET['cart'] ?>&sum=<?php echo $_GET['sum'] ?>">
                < arrière</a>
        </div>
        <div class="col-md-12" style="margin: auto;float: unset;padding-left:0;padding-right:0">
            <?php
            ...
            <div class="col-md-12">
                ...
                <table class="table">
                    <thead>
                        ...
                        <th style="width: 95px;display: none">Stripe ID</th>
                        <th style="width: 400px;">Type</th>
                        <!--<th></th>-->
                        <!--<th style="width: 100px;">Amount</th>-->
                        <th style="width: 50px;">Articles</th>
                        <th style="width: 65px;">Taxes</th>
                        <th style="width: 50px;">Discount</th>
                        <th style="width: 75px;">Cout</th>
                        <th style="width: 70px;visibility: hidden">Boxes</th>
                        <th style="width: 50px;">Articles</th>
                        <th style="width: 65px;">Taxes</th>
                        <th style="width: 50px;">Discount</th>
                        <th style="width: 75px;">Cout</th>
                        <th style="width: 50px;visibility: hidden">Boxes</th>
                        <!--<th></th>-->
                        <th>Balance</th>
                    ...
                    <div class="date" style="display: flex;flex-wrap: wrap;">
                        ...
                        ?>
                            ...
                            <div class="ord" style="display: flex">
                                ...
                                <div>
                                    <div style="width: 410px; border-left: 1px solid #ccc;padding-left: 5px;border-right: 1px solid #ccc;border-top: 1px solid #ccc"><?php if ($rows2['payment_type'] == 'products') {
                                                                                                                                                                            echo 'Paiement / Dēpōt';
                                                                                                                                                                        } else {
                                                                                                                                                                            echo $rows2['payment_type'];
                                                                                                                                                                        } ?></div>
                                </div>

                                <?php if ($rows2['payment_type'] == 'products') { ?>

                                    <div class="bodd" style="width: 782px;border-top: 1px solid #ccc;border-right: 1px solid #ccc">
                                        <div style="width: 325px;display: flex;justify-content: space-between;;margin-left: 10px;height: 100%;visibility: hidden" class="amt" data-amt="<?php echo $rows2['paid_amount']; ?>">
                                            <?php echo number_format($wish_price, 2); ?>€
                                            <div>
                                                <div style="width: 50px"><?php echo number_format($wish_price * 0.17, 2); ?>€</div>
                                            </div>
                                            <div>
                                                <div style="width: 50px">-<?php echo number_format($discounted_price, 2); ?>€</div>
                                            </div>
                                            <div>
                                                <div style="width: 50px"><?php echo $rows2['paid_amount']; ?>€</div>
                                            </div>
                                            <div style="display: flex; align-items: flex-end">
                                                <div style="width: 50px"><?php echo number_format($exp, 2); ?>€</div>
                                            </div>
                                        </div>
                            ...
                            ?>
                                <div class="ord tst" style="display: flex">
                                    <?php
                                    $discounted_amt_price = 0;

                                    if (!empty($rows_w['products'])) {
                                        $pros = explode(",", $rows_w['products']);
                                        $pro_name = '';
                                        $discounted_price = 0;
                                        $exp = 0;
                                        ...
                                        foreach ($pros as $pr) {
                                            ...
                                            $result_d = Helper::dbQuery('SELECT * FROM seller where seller_name LIKE  "%' . $get_data_p['source'] . '%"');
                                            // print_r($sql_d);

                                            $get_data_d = $result_d[0];

                                            $parcel_weight = $get_data_p['parcel_weight'];
                                            $parcel_l = $get_data_p['parcel_l'];
                                            $parcel_b = $get_data_p['parcel_b'];
                                            $parcel_h = $get_data_p['parcel_h'];
                                            $parcel_vol = $parcel_l * $parcel_b * $parcel_h / 5000;
                                            if ($parcel_weight > $parcel_vol) {
                                                $parcel_weight = $parcel_weight;
                                            } else {
                                                $parcel_weight = $parcel_vol;
                                            }

                                            $parcel_pri = 0;
                                            if ($parcel_weight > 0) {
                                                $result_weight = Helper::dbQuery('SELECT * FROM shipping_weight_prices WHERE weight = "' . $parcel_weight . '"');

                                                $get_basic_weight_data = $result_weight[0];
                                                $shippp = $get_data_p['selected_shipping'];

                                                $parcel_pri = $get_basic_weight_data[$shippp];
                                            }
                                            if (!empty($get_data_d)) {
                                                if ($get_data_d['discount_type'] == 'Percentage') {
                                                    if ($counter == 0) {
                                                        $discounted_amt_price = ($get_data_d['discount_value'] / 100) * $amt;
                                                    }
                                                    $pro_price = ($get_data_p['price'] * $get_data_p['qty']);
                                                    $discounted_price += ($get_data_d['discount_value'] / 100) * $pro_price;
                                                    $dis_p = ($get_data_d['discount_value'] / 100) * $get_data_p['price'];
                                                } else {
                                                    if ($counter == 0) {
                                                        $discounted_amt_price = $get_data_d['discount_value'];
                                                    }
                                                    $pro_price = ($get_data_p['price'] * $get_data_p['qty']);
                                                    $discounted_price += $get_data_d['discount_value'];
                                                }
                                    ...
                                    <div>
                                        <div data-pross="<?php echo $rows_w['products']; ?>" class="pros" title="<?php echo $rows_w['ajouter']; ?>" data-id_cart="<?php echo $rows_w['id_cart']; ?>" data-date="<?php echo date('d/m/Y', strtotime($date)); ?>" data-id="<?php echo $rows_w['id']; ?>" data-ajouter="<?php echo $rows_w['ajouter']; ?>" data-de_client="<?php echo $rows_w['de_client']; ?>" data-re_client="<?php echo $rows_w['re_client']; ?>" class="pop_edit" style="width: 410px; word-wrap: normal;display:block;cursor: pointer;">
                                            ...
                                            if (!empty($rows_w['products'])) {
                                                ...
                                                foreach ($pros as $pr) {
                                                    ...
                                                    $exp += $ress['selected_shipping_price'];
                                                    $selectedship = $ress['selected_shipping'];
                                                    $pro_pri += $get_data_p['price'] * $get_data_p['qty'];
                                                    $master = $ress['master_weight'];
                                                    $length = $ress['length'];
                                                    $width = $ress['width'];
                                                    $height = $ress['height'];
                                                    ...
                                                    if (round($master, 1) == 2) {
                                                        $showwgt = round($master, 1);
                                                        $len = '30';
                                                        $wid = '20';
                                                        $hgt = '15';
                                                    } else if (round($master, 1) == 8) {
                                                        $showwgt = round($master, 1);
                                                        $len = '50';
                                                        $wid = '40';
                                                        $hgt = '20';
                                                    } else if (round($master, 1) == 10) {
                                                        $showwgt = round($master, 1);
                                                        $len = '60';
                                                        $wid = '40';
                                                        $hgt = '20';
                                                    } else if (round($master, 1) == 16) {
                                                        $showwgt = round($master, 1);
                                                        $len = '50';
                                                        $wid = '40';
                                                        $hgt = '40';
                                                    } else {
                                                        $difff = round($master, 1) / 3;
                                                        if (!empty($ress['length'])) {
                                                            $showwgt = $ress['weight'];
                                                            $len = $ress['length'];
                                                        } else {
                                                            $len = '';
                                                    ...
                                                    $pro_name .= "<span style='display: block;border: 1px solid #ccc;border-left: unset;border-right: unset;'>$product_name $product_price</span><br>";
                                            ...
                                            ?>
                                                <span style="font-style: italic">Service <?php if ($selectedship) {
                                                                                                echo chop($selectedship, '_price');
                                                                                            } else {
                                                                                                echo 'dhl';
                                                                                            }; ?><?php echo ' ' . $master . ' Kg - ' . $len . ' x ' . $wid . ' x ' . $hgt . ' cm'; ?></span>
                                                <br>
                                                <?php echo $rows_w['ajouter']; ?>
                                    ...
                                    <?php if (!empty($rows_w['products'])) { ?>
                                        ...
                                        <div style="border-top: 1px solid #ccc;">
                                            <div style="width: 325px;display: flex;justify-content: space-between;height: 100%;margin-top: 20px;border-top: 1px solid #ccc;" data-amt="<?php echo number_format($discounted_price + $pro_pri + ($pro_pri * 0.17), 2) ?>"><?php echo $pro_pri; ?>€<div style="border-left: 1px solid #ccc;border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($pro_pri * 0.17, 2); ?>€</div>
                                                </div>
                                                <div style="border-right: 1px solid #ccc;">
                                                    <div style="width: 50px">-<?php echo number_format($discounted_price, 2); ?>€</div>
                                                </div>
                                                <div>
                                                    <div style="width: 100px;margin-top: -20px"><span style="font-style:italic"><?php echo $exp; ?>€ <?php echo ' (' . $master . ' Kg)'; ?></span><br><span style="font-weight: bold"><?php echo number_format($discounted_price + $pro_pri + ($pro_pri * 0.17), 2) ?>€</span></div>
                                                </div>
                                                <div style="display: flex; align-items: flex-end">
                                                    <div style=""></div>
                                                </div>
                                    ...
                                    <?php } else if (!empty($rows_w['re_client'])) { ?>
                                        <div style="border-top: 1px solid #ccc;">
                                            <div style="width: 325px;display: flex;justify-content: space-between;height: 100%;margin-top: 20px;border-top: 1px solid #ccc;" class="amt" data-amt="<?php echo number_format($discounted_price + $pro_pri + ($pro_pri * 0.17), 2) ?>"><?php echo $pro_pri; ?>€<div style="border-left: 1px solid #ccc;border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($amt * 0.17, 2); ?>€</div>
                                                </div>
                                                <div style="border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($discounted_price, 2); ?>€</div>
                                                </div>
                                    ...
                                    ?>
                                        <div></div>
                                        <div class="bal" style="border-top: 1px solid #ccc;border-right: 1px solid #ccc">
                                            <div style="width: 455px;display: flex;justify-content: space-between;padding-left: 30px;height: 100%;padding-right: 16px;margin-top: 20px;border-top: 1px solid #ccc;" class="amt sub" data-amt="<?php echo number_format($discounted_amt_price + $amt + ($amt * 0.17), 2) ?>"><?php echo $amt; ?>€<div style="border-left: 1px solid #ccc;border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($amt * 0.17, 2); ?>€</div>
                                                </div>
                                                <div style="border-right: 1px solid #ccc;">
                                                    <div style="width: 50px">-<?php echo number_format($discounted_amt_price, 2); ?>€</div>
                                                </div>
                                                <div>
                                                    <div style="width: 85px;margin-top: -20px"><span style="font-style: italic"><?php echo $parcel_pri; ?>€ <?php echo ' (' . $parcel_weight . ' Kg)' ?></span><br><span style="font-weight: bold"><?php echo number_format(-$discounted_amt_price + $amt + ($amt * 0.17), 2) ?>€</span></div>
                                                </div>
                                                <div class="bal" style="margin-top: -20px;text-align: right">
                                                    <div style="width: 70px"><?php echo $exp - $parcel_pri; ?>€</div>
                                                </div>
                                                <div style="display: flex; align-items: flex-end">
                                                    <div style=""></div>
                                                </div>
                                    ...
                                    <?php } else if (!empty($rows_w['de_client'])) { ?>
                                        <div class="bal" style="border-top: 1px solid #ccc;border-right: 1px solid #ccc;">
                                            <div style="width: 325px;display: flex;justify-content: space-between;height: 100%;margin-top: 20px;border-top: 1px solid #ccc;;" class="amt sub" data-amt="<?php echo number_format($discounted_amt_price + $amt + ($amt * 0.17), 2) ?>"><?php echo $amt; ?>€<div style="border-left: 1px solid #ccc;border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($amt * 0.17, 2); ?>€</div>
                                                </div>
                                                <div style="border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($discounted_amt_price, 2); ?>€</div>
                                    ...
                                    <?php } else if (!empty($rows_w['re_client'])) { ?>
                                        <div class="bal" style="border-top: 1px solid #ccc;border-right: 1px solid #ccc;">
                                            <div style="width: 325px;display: flex;justify-content: space-between;height: 100%;margin-top: 20px;border-top: 1px solid #ccc;" class="amt" data-amt="<?php echo number_format($discounted_amt_price + $amt + ($amt * 0.17), 2) ?>"><?php echo $amt; ?>€<div style="border-left: 1px solid #ccc;border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($amt * 0.17, 2); ?>€</div>
                                                </div>
                                                <div style="border-right: 1px solid #ccc;">
                                                    <div style="width: 50px"><?php echo number_format($discounted_amt_price, 2); ?>€</div>
                                    ...
                                    <?php } else { ?>
                                        <!--<div><div style="width: 100px;"></div></div>-->
                                    <?php } ?>
                                    <div class="pri" style="padding-left: 30px;border: 1px solid #ccc;border-left: unset;padding-right: 30px;border-bottom: unset"></div>
                                </div>
                        <?php
                            }
                        }
...
<script>
    $(document).ready(function() {
        var hgt = parseFloat($('.qwqw').height());
        $('.bal').append('<style>.bal:before{height: calc(' + hgt + 'px / 3) !important}</style>')
        //   var array = [];
        // $('.ddte').each(function(){
