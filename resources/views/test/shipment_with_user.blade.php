        ...
        <button type="button" class="btn btn-pdf" name="button" style="display: none">Generate PDF</button>
           <div class="col-md-12 boxes" style="margin: unset">
              ...
              <table id="example14" class="display" style="width:100%">
            	...
            	$idd = $_GET['id'];
            	$res = Helper::dbQuery('SELECT * FROM customer_product_wishlist WHERE parcel_number IS NOT NULL AND parcel_for=1 AND is_archived IS NULL AND id_customer='.$idd.' GROUP BY parcel_number ORDER BY id DESC');
            			...
            			<tr>
            			    ...
            			    <th style="display: flex;align-items: center;justify-content: space-between">
            						...
            						<select class="form-control" id="send_parcel" data-value="<?php echo $id; ?>" style="float:right;width: 90px;font-size: 13px;padding: 5px;">
            						    ...
            						    <option value="1" <?php if($result['parcel_for'] == 1){echo 'selected';} ?>>Colisrael</option>
            						    <option value="2" <?php if($result['parcel_for'] == 2){echo 'selected';} ?>>Personal</option>
            						</select>
            				</th>
            			</tr>

                		<tr style="display: none"><td><?= $test?></td></tr>
            			<?php
            			$counter++;
                    	    }
        ...
        <div class="col-md-4">
           <div class="col-md-12 boxes" style="margin: unset">
              <table id="example14" class="display" style="width:100%">
            	...
            	$res = Helper::dbQuery('SELECT * FROM customer_product_wishlist WHERE parcel_number IS NOT NULL AND parcel_for=0 AND is_archived IS NULL AND id_customer='.$idd.' GROUP BY parcel_number ORDER BY id DESC');
        ...
        <button type="button" class="btn btn-pdf_personal" name="button" style="display: none">Generate PDF</button>
           <div class="col-md-12 boxes" style="margin: unset">
              <table id="example14" class="display" style="width:100%">
            	...
            	<?php
            	$domain_url = env('APP_URL');
            	$res = Helper::dbQuery('SELECT * FROM customer_product_wishlist WHERE parcel_number IS NOT NULL AND parcel_for=2 AND is_archived IS NULL AND id_customer='.$idd.' GROUP BY parcel_number ORDER BY id DESC');
...
// })
    ...
    function change_parcel_status(id, status){
        ...
        $.ajax({
         type: "POST",
         url: '{{ env('APP_URL') }}/change_parcel_type.php',
    ...
    // Set a Cookie
     ...
     function updateStatus(id) {
      $.ajax({
         type: "POST",
         url: '{{ env('APP_URL') }}/update_invoice_status.php',
         data: {id: id},
         success: function(data){
